...
////
Explain that nosql systems are heterogenous so there is no single point of access and each engine implement their own.
Means that each database supported by nosqlunit has its own dataset format (we put two or three quick examples)
Nosqlunit can manage lifecycle of the database (embed, managed, remote)
Example
////
